...
class Simulator:
    ...
    def load(self, scene, seed=None):
        self.remote.command("simulator/load_scene",
                    ...
                    if event_type in callbacks:
                        for fn in callbacks[event_type]:
                            if event_type == "collision":
                                fn(agent, self.agents.get(ev["other"]), Vector.from_json(
                                    ev["contact"]) if ev["contact"] is not None else None)
                            ...
                            elif event_type == "destination_reached":  # added by sunjianlei
                                fn(agent)  # added by sunjianlei
                            if self.stopped:
                                return
    ...
    def weather(self, state):
        self.remote.command("environment/weather/set",
                            {"rain": state.rain,
                             "fog": state.fog,
    ...
    def set_time_of_day(self, time, fixed=True):
        self.remote.command("environment/time/set",
    ...
    def map_to_gps(self, transform):
        j = self.remote.command("map/to_gps",
    ...
    def map_from_gps_batch(self, coords):
        ...
        for c in coords:
            j = {}
            numtype = (int, float)
            if ("latitude" in c and c["latitude"] is not None) and (
                    "longitude" in c and c["longitude"] is not None):
                if not isinstance(c["latitude"], numtype):
                    raise TypeError(
                        "Argument 'latitude' should have '{}' type".format(numtype))
                if not isinstance(c["longitude"], numtype):
                    raise TypeError(
                        "Argument 'longitude' should have '{}' type".format(numtype))
                if c["latitude"] < -90 or c["latitude"] > 90:
                    raise ValueError("Latitude is out of range")
                if c["longitude"] < -180 or c["longitude"] > 180:
                ...
                if not isinstance(c["northing"], numtype):
                    raise TypeError(
                        "Argument 'northing' should have '{}' type".format(numtype))
                if not isinstance(c["easting"], numtype):
                    raise TypeError(
                        "Argument 'easting' should have '{}' type".format(numtype))
                if c["northing"] < 0 or c["northing"] > 10000000:
                    raise ValueError("Northing is out of range")
                if c["easting"] < 160000 or c["easting"] > 834000:
                    raise ValueError("Easting is out of range")
                j["northing"] = c["northing"]
                j["easting"] = c["easting"]
            else:
                raise Exception(
                    "Either latitude and longitude or northing and easting should be specified")
            if "altitude" in c and c["altitude"] is not None:
                if not isinstance(c["altitude"], numtype):
                    raise TypeError(
                        "Argument 'altitude' should have '{}' type".format(numtype))
                j["altitude"] = c["altitude"]
            if "orientation" in c and c["orientation"] is not None:
                if not isinstance(c["orientation"], numtype):
                    raise TypeError(
    ...
    def map_point_on_lane(self, point):
        j = self.remote.command("map/point_on_lane",
                                {"point": point.to_json()})
    ...
    def raycast(
        ...
        if hit[0] is None:
            return None
        return RaycastHit(
            hit[0]["distance"], Vector.from_json(
                hit[0]["point"]), Vector.from_json(
                hit[0]["normal"]))
    ...
    def raycast_batch(self, args):
        ...
        for hit in hits:
            if hit is None:
                ...
                results.append(
                    RaycastHit(
                        hit["distance"], Vector.from_json(
                            hit["point"]), Vector.from_json(
                            hit["normal"])))
    ...
    def controllable_remove(self, controllable):
        self.remote.command("simulator/controllable_remove",
