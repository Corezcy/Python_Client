...
GpsData = namedtuple(
    "GpsData",
    "latitude longitude northing easting altitude orientation")
...
class Sensor:
    ...
    def enabled(self, value):
        self.remote.command("sensor/enabled/set",
                            {"uid": self.uid, "enabled": value})
...
class LidarSensor(Sensor):
    def __init__(self, remote, j):
        ...
        # how many measurements each ray does per one rotation
        self.measurements = j["measurements"]
        self.fov = j["fov"]
        self.angle = j["angle"]
