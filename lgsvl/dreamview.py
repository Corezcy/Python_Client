...
class Connection:
    ...
    def __init__(self, simulator, ego_agent, ip="localhost", port="8888"):
        ...
        self.gps_offset = lgsvl.geometry.Vector()
    ...
    def set_destination(
        ...
        # Unity returns the position of the center of the vehicle so adjustment
        # is required

        northing_adjustment = (

            math.sin(heading) * self.gps_offset.z -
            math.cos(heading) * self.gps_offset.x

        )

        easting_adjustment = (

            math.cos(heading) * self.gps_offset.z +
            math.sin(heading) * self.gps_offset.x

        )

        if coord_type == CoordType.Unity:

            transform = lgsvl.geometry.Transform(
                lgsvl.geometry.Vector(
                    x_long_east, y, z_lat_north), lgsvl.geometry.Vector(
                    0, 0, 0))
            ...
            transform = self.sim.map_from_gps(
                longitude=x_long_east, latitude=z_lat_north)
    ...
    def set_hd_map(self, map):
        ...
        self.ws.send(
            ...
            json.dumps(
                {"type": "HMIAction", "action": "CHANGE_MAP", "value": map})
    ...
    def set_vehicle(
        ...
        # Lincoln2017MKZ from LGSVL has a GPS offset of 1.348m behind the
        # center of the vehicle, lgsvl.geometry.Vector(0.0, 0.0, -1.348)
        """
    ...
    def get_module_status(self):
        ...
        # This first recv() call returns the SimControlStatus in the form
    ...
    def setup_apollo(self, dest_x, dest_z, modules):
        ...
        # If any module is not enabled, Control may still send a message even
        # though Apollo is not ready

        # if not all(mod_status[mod] for mod in modules):   #被孙建蕾注释掉

        #     self.disable_apollo()                         #被孙建蕾注释掉
        ...
        for i in range(20):
            ...
            log.error(
                "No control message from Apollo within 40 seconds. Aborting...")
