...
class EgoVehicle(Vehicle):
    ...
    def set_fixed_speed(self, isCruise, speed=None):
        self.remote.command("vehicle/set_fixed_speed",
                            {"uid": self.uid,
                             "isCruise": isCruise,
                             "speed": speed})
    ...
    def destination_set(self, destination=None):
        self.remote.command("agent/destination/set",
                            {"uid": self.uid,
...
class NpcVehicle(Vehicle):
    ...
    # this function is addded by sunjianlei
    @accepts(Iterable, bool)
    def follow_waypoints_jsonfile(self, waypoints, loop=False):
        self.remote.command(
                ...
                "waypoints": [
                    {
                        ...
                        "deactivate": False,
                        "trigger_distance": (
                            None if wp['trigger']['effectors'] == [] else wp['trigger']['effectors'][0]['parameters']['max_distance']),
                        "timestamp":-1,
                        "trigger":None} for wp in waypoints],
                "loop": loop,
            })

    def follow_closest_lane(self, follow, max_speed, isLaneChange=True):
        self.remote.command("vehicle/follow_closest_lane",
                            {"uid": self.uid,
                             "follow": follow,
                             "max_speed": max_speed,
    ...
    def set_behaviour(self, behaviour):
        self.remote.command("vehicle/behaviour",
                            {"uid": self.uid, "behaviour": behaviour})

    @accepts(bool)
    def change_lane(self, isLeftChange):
        self.remote.command("vehicle/change_lane",
                            {"uid": self.uid, "isLeftChange": isLeftChange})
...
class Pedestrian(Agent):
    ...
    def walk_randomly(self, enable):
        self.remote.command("pedestrian/walk_randomly",
                            {"uid": self.uid, "enable": enable})
    ...
    def follow_waypoints_jsonfile(self, waypoints, loop=False):
        self.remote.command("pedestrian/follow_waypoints", {
            ...
            "waypoints": [{
                ...
                "idle": wp['wait_time'],
                "trigger_distance": (
                    None if wp['trigger']['effectors'] == [] else wp['trigger']['effectors'][0]['parameters'][
                        'max_distance']),
                "trigger": None
            } for wp in waypoints],
            "loop": loop,
    ...
    def set_speed(self, speed):
        self.remote.command("pedestrian/set_speed",
                            {"uid": self.uid, "speed": speed})
